...
LONG_OUTPUT = """Test arguments are: (LONG " " " ")\r
...
class ExternalScriptTest(BasicTest):
	...
	def do_one_test(self, script, expected = status.OK, message = None, args=[]):
	...
	def do_one_language_test_noargs(self, language):
		result = TestResult('Doing tests: %s'%language)
		result.add(self.do_one_test('tes_script_%s'%language, message='Test arguments are: (  )'))
		result.add(self.do_one_test('tes_sa_%s'%language, status.OK, 'Test arguments are: (ARG1 "ARG 2" "A R G 3")'))
		result.add(self.do_one_test('tes_script_%s'%language, status.UNKNOWN, 'Arguments not allowed', ['NOT ALLOWED']))
		return result
		
	def do_one_language_test_args(self, language):
		result = TestResult('Doing tests: %s'%language)
		result.add(self.do_one_test('tes_sca_%s'%language, status.OK, None, ['OK']))
		result.add(self.do_one_test('tes_sca_%s'%language, status.WARNING, None, ['WARN']))
		result.add(self.do_one_test('tes_sca_%s'%language, status.CRITICAL, None, ['CRIT']))
		result.add(self.do_one_test('tes_sca_%s'%language, status.UNKNOWN, None, ['UNKNOWN']))
		result.add(self.do_one_test('tes_sca_%s'%language, status.OK, 'Test arguments are: (OK "String with space" "A long long option with many spaces")', ['OK', 'String with space', 'A long long option with many spaces']))
		if language == "ps1":
			result.add(self.do_one_test('tes_sca_%s'%language, status.OK, LONG_OUTPUT.replace("\r",""), ['LONG', ' ', ' ']))
		else:
			result.add(self.do_one_test('tes_sca_%s'%language, status.OK, LONG_OUTPUT, ['LONG', ' ', ' ']))
		result.add(self.do_one_test('tes_sca_%s'%language, status.UNKNOWN, 'Request contained illegal characters set /settings/external scripts/allow nasty characters=true!', ['OK', '$$$ \\ \\', '$$$ \\ \\']))
		return result

	def do_one_language_test_nasty_args(self, language):
		result = TestResult('Doing tests: %s'%language)
		result.add(self.do_one_test('tes_sca_%s'%language, status.OK, 'Test arguments are: (OK "$$$ \\ \\" "$$$ \\ \\")', ['OK', '$$$ \\ \\', '$$$ \\ \\']))
		return result
	...
	def run_test(self):
		...
		result.add(self.do_one_language_test_noargs('bat'))
		result.add(self.do_one_language_test_noargs('ps1'))
		...
		result.add(self.do_one_language_test_args('bat'))
		result.add(self.do_one_language_test_args('ps1'))
		...
		result.add(self.do_one_language_test_nasty_args('bat'))
		#result.add(self.do_one_language_test_nasty_args('ps1'))
	...
	def install(self, arguments):
		...
		conf.set_string('/settings/test_external_scripts/scripts', 'tes_sa_ps1', 	'cmd /c echo scripts\\check_test.ps1 ARG1 "ARG 2" "A R G 3"; exit($lastexitcode) | powershell.exe -command -')
		...
		conf.set_string('/settings/test_external_scripts/scripts', 'tes_sca_ps1', 	'cmd /c echo scripts\\check_test.ps1 $ARG1$ "$ARG2$" "$ARG3$"; exit($lastexitcode) | powershell.exe -command -')
